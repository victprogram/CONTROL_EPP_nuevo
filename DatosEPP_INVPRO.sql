...
insert  into `fecha`(`id`,`fechainicio`,`fechafinal`,`idfactura`,`codigoautorizacion`,`nombrecategoria`,`marca`,`idempleado`,`nombrearea`,`idarea`,`cargo`) values (1,'2020-07-29 00:00:00','2020-08-26 23:59:59',24,83817872,'CAMISA','EJEMPLO',1838,'PRINCIPAL',3,'MODERADOR');
...
insert  into `principal`(`id`,`nombreempresa`,`direccionempresa`,`telefonoempresa`,`correo`,`correoaprobacion`,`rnc`,`urlconsulta`,`urlsalida`,`urlempleado`,`impresora`) values (1,'AGENCIA NAVARRO','C/ Basilio Gavilán, esq. Respaldo Antón, Sector la Yuca, Cotui, Prov. Sánchez Ramírez, R. D.','(809)240-1107','info@agencianavarro.com','tecnologia@agencianavarro.com','-','http://navarrowindows.ddns.net/bprestservices/getwebcatalogpaginado.rsvc?incluircosto=si?pagina=1&rowsPorPagina=3000%20HTTP/1.1','http://navarrowindows.ddns.net/bprestservices/invsalida.rsvc','http://navarrowindows.ddns.net/bprestservices/empleado.rsvc?pagina=1&rowsPorPagina=1000 HTTP/1.1','');
...
BEGIN
START TRANSACTION;
INSERT INTO facturas(idusuario,idempleado,Cedula,NombreEmpleado,AreaTrabajo,NombrePO,codigoaprovacion,observacion)
VALUES (_idusuario, _idempleado,_Cedula,_NombreEmpleado,_AreaTrabajo,_NombrePO,_codigoaprovacion,_observacion);     
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `reporteeppplano` AS (select `detallesfacturas`.`iddetallesfactura` AS `iddetallesfactura`,`detallesfacturas`.`idfactura` AS `idfactura`,`facturas`.`NombrePO` AS `NombrePO`,`facturas`.`NombreEmpleado` AS `Nombre`,`facturas`.`Cedula` AS `Cedula`,`facturas`.`AreaTrabajo` AS `AreaTrabajo`,`facturas`.`fecha` AS `fecha`,`detallesfacturas`.`idproducto` AS `idproducto`,`detallesfacturas`.`nombreepp` AS `nombreepp`,`detallesfacturas`.`cantidad` AS `cantidad`,`detallesfacturas`.`precio` AS `precio`,`detallesfacturas`.`subtotal` AS `subtotal`,`detallesfacturas`.`tiempovida` AS `tiempovida`,`detallesfacturas`.`fechavence` AS `fechavence`,`principal`.`nombreempresa` AS `nombreempresa`,`principal`.`direccionempresa` AS `direccionempresa`,`principal`.`telefonoempresa` AS `telefonoempresa`,`principal`.`correo` AS `correo`,`principal`.`rnc` AS `rnc`,concat(`usuarios`.`nombre`,' ',`usuarios`.`apellido`) AS `usuario`,`fecha`.`fechainicio` AS `fechainicio`,`fecha`.`fechafinal` AS `fechafinal`,`detallesfacturas`.`codigoaprovacion` AS `codigoaprovacion` from (((((`facturas` join `detallesfacturas`) join `usuarios`) join `fecha`) join `principal`) join `po`) where ((`facturas`.`idempleado` = `detallesfacturas`.`idempleado`) and (`facturas`.`idfactura` = `detallesfacturas`.`idfactura`) and (`facturas`.`idusuario` = `usuarios`.`idusuario`) and (`facturas`.`fecha` between `fecha`.`fechainicio` and `fecha`.`fechafinal`))) */;
